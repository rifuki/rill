//! The gas questions `gas_live.rs` leaves open.
//!
//! A) Does `do_gas_selection` REPAIR a stale gas object ref, or only fill an empty one?
//! B) With it on, is a price below the reference still a hard gRPC error?
...
//!
//! Open, and worth closing: gas refs go stale between the three transactions a full flow needs,
